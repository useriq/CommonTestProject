...
public class ElementWeb {
    ...
    public void setCursorRihgtSideAndClickOtherElement(Dimension size, By by) {
        Actions actions = new Actions(getWebDriver());
        actions.moveToElement(object,size.getWidth()-1,size.getHeight()-1).click($(by));
        actions.build().perform();
    }

    public void setCursorAndClick() {
        Actions actions = new Actions(getWebDriver());
        actions.moveToElement(object).click().build().perform();
    }
